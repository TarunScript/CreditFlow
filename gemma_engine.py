...
def _parse_unstructured_fallback(raw_text):
    """High-precision deterministic regex & keyword parser for unstructured text statements."""
    lines = [ln.strip() for ln in raw_text.strip().split("\n") if ln.strip()]
    records = []
    for i, line in enumerate(lines):
        clean = line.replace(",", "")
        # Pattern matching for semantic keywords (inflow/income/credit and outflow/expense/debit)
        inc_m = re.search(r'(?:income|inflow|earned|credit|in)\D*?(\d+(?:\.\d+)?)', clean, re.IGNORECASE)
        exp_m = re.search(r'(?:expense|expenses|outflow|spent|debit|out)\D*?(\d+(?:\.\d+)?)', clean, re.IGNORECASE)
        m_m = re.search(r'(?:month|m)\D*?(\d+)', clean, re.IGNORECASE)
        m_num = int(m_m.group(1)) if m_m else (i + 1)

        if inc_m and exp_m:
            records.append({"month": m_num, "income": float(inc_m.group(1)), "expenses": float(exp_m.group(1))})
        else:
            # Fallback to positional numbers
            nums = re.findall(r"\d+(?:\.\d+)?", clean)
            if len(nums) >= 3 and float(nums[0]) <= 36 and float(nums[1]) > 500:
                records.append({"month": int(nums[0]), "income": float(nums[1]), "expenses": float(nums[2])})
            elif len(nums) >= 2:
                records.append({"month": i + 1, "income": float(nums[0]), "expenses": float(nums[1])})

    if records:
        return records, None
    return None, "Unable to parse statement text into structured monthly records. Expected format: 'Month 1: Inflow Rs 22000, Outflow Rs 11500'."
...
def extract_financial_records(raw_text, model=DEFAULT_MODEL, host=DEFAULT_HOST):
    ...
    try:
        ...
        return _parse_unstructured_fallback(raw_text)
    except (OllamaUnavailable, json.JSONDecodeError, ValueError, TypeError, Exception):
        return _parse_unstructured_fallback(raw_text)
